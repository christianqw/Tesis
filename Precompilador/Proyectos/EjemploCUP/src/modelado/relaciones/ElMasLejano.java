...
public class ElMasLejano extends Evaluador{
    ...
    public Elemento evaluar() {
        /*Recorremos todos los elementos del dominio, buscando el que se encuentra mas lejos al 
    ...
    private Elemento buscar(ArrayList<Elemento> instancia, Elemento elem) {        
        ...
        for (int i = 0; i < instancia.size() ; i++) {
            ...
            if (! instancia.get(i).getNombre().equals(elem.getNombre())){
                ...
                if ((distanciaMayor < dTemporal) || (distanciaMayor == 0)){
                    distanciaMayor = dTemporal;
